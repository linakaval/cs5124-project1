...
d3.csv('data/disasters.csv')
  ...
  for(let i = 0; i<data.length; i++){
  	...
  	let cost = +Number(data[i].cost);// convert string 'cost' to number
  	...
  	let radius = mapFromDomainToRange(1, 160, 0, 365, cost); //******* don't use 10, use mapFromDomainToRange.  The max cost is 160, and the min cost is 1
...
// here we add a circle element to the svg in the DOM
  ...
  function writeYear(year, xPos, yPos, svg){
	const svgns = "http://www.w3.org/2000/svg"; 
		let newText = document.createElementNS(svgns, "text");
		newText.setAttribute("x", xPos);
		newText.setAttribute("y", yPos+5);
		newText.setAttribute("fill", "black");
		//newText.setAttribute("value", year)
		newText.appendChild(document.createTextNode(year));
		// append the new line to the svg
